...
class TweetParser {
  constructor(str) {
    ...
    this._potentialSentenceCount = 0;
    this._endSentenceCharacterCount = 0;
    this._startSentenceCharacterCount = 0;
    this._phrases = [];
    this._lastPhrase = '';
    ...
    for (let i = 0; i < str.length; i++) {
      ...
      if (detector.shouldEndSentence(str[i]) &&
        !detector.shouldEndSentence(str[i + 1]) // ignore consecutive sentence-end characters
        || !str[i + 1]) {
          this._potentialSentenceCount += 1;
      }

      if (detector.shouldEndSentence(str[i]) &&
        !detector.shouldEndSentence(str[i + 1])) {
          this._endSentenceCharacterCount += 1;
      }

      if (!str[i - 1]
        || detector.isUpperCaseCharacter(str[i]) && str[i - 2]
        && str[i - 1] && detector.isSpace(str[i -1])
        && detector.shouldEndSentence(str[i - 2])
      ) {
        this._startSentenceCharacterCount += 1;
      }

      if (detector.isSpace(str[i]) && !this._lastPhrase.length) {
        continue; // we don't want to begin a phrase with a space character, so skip adding until we've begun the phrase with a non-space character
      } else {
        this._lastPhrase += str[i];
      }

      if (detector.shouldEndSentence(str[i]) && !detector.shouldEndSentence(str[i + 1]) || !str[i + 1]) {
        this._phrases.push(this._lastPhrase);
        this._lastPhrase = '';
      }
    }
  }
  ...
  potentialSentenceCount() {
    return this._potentialSentenceCount;
  }

  endSentenceCharacterCount() {
    return this._endSentenceCharacterCount;
  }

  hasBadStructure() {
    if (this._startSentenceCharacterCount != this._endSentenceCharacterCount
      || this._hasConsecutiveUpperCaseLetters) {
  ...
  startSentenceCharacterCount() {
    return this._startSentenceCharacterCount;
  }
